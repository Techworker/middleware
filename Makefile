...
init:
	docker build -t middleware ./resources/docker

docker:
	docker run -it --rm -e LOCAL_USER_ID=`id -u` -e XDEBUG_CONFIG="remote_host=dockerhost" -e PHP_IDE_CONFIG="serverName=middleware" --name middleware --add-host dockerhost:$(host) -v $(pwd):/usr/src -w /usr/src middleware $(args)
